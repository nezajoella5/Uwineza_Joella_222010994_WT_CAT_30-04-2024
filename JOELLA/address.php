...
<section>
    ...
    <h1><u> address Form </u></h1>
    <form method="post">
            
        <label for="aid">addressid:</label>
        ...
        <input type="text" id="bal" name="ctry" required><br><br>
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $stmt = $connection->prepare("INSERT INTO address(addressid, customerid, city,country) VALUES (?, ?, ?, ?)");
    $stmt->bind_param("ssss", $aid, $cid, $cty, $ctry);
...
// SQL query to fetch data from the address table
...
<head>
    <meta charset="utf-8">
    <meta name="viewport" content="width=device-width, initial-scale=1">
    <title>address of the customer</title>
    <style>
        table {
            width: 100%;
...
<body>
    <center><h2>Table of address</h2></center>
    <table border="5">
        <tr>
            <th>address id</th>
            <th>customer id</th>
        ...
        // Prepare SQL query to retrieve all address
        $sql = "SELECT * FROM address";
        $result = $connection->query($sql);
        ...
        if ($result->num_rows > 0) {
            ...
            while ($row = $result->fetch_assoc()) {
                $aid = $row['addressid']; // Fetch the addressid
                echo "<tr>
                    <td>" . $row['addressid'] . "</td>
                    <td>" . $row['customerid'] . "</td>
                    <td>" . $row['city'] . "</td>
                    <td>" . $row['country'] . "</td>
                    <td><a style='padding:4px' href='delete_address.php?addressid=$aid'>Delete</a></td> 
                    <td><a style='padding:4px' href='update_address.php?addressid=$aid'>Update</a></td> 
                </tr>";
            }
        } else {
